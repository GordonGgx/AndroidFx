...
public final class LocalBroadcastManager {
    ...
    public void registerReceiver( BroadcastReceiver receiver,
        ...
        synchronized (mReceivers) {
            ...
            ArrayList<ReceiverRecord> filters = mReceivers.computeIfAbsent(receiver, k -> new ArrayList<>(1));
            filters.add(entry);
            for (int i=0; i<filter.countActions(); i++) {
                String action = filter.getAction(i);
                ArrayList<ReceiverRecord> entries = mActions.computeIfAbsent(action, k -> new ArrayList<>(1));
                entries.add(entry);
            }
        }
    }
        ...
        synchronized (mReceivers) {
            ...
            if (entries != null) {
                ...
                for (int i=0; i<entries.size(); i++) {
                    ...
                    boolean match = receiver.filter.matchAction(action);
                    if (match) {
                        if (receivers == null) {
